...
import com.bonlimousin.replica.config.KafkaProperties;
...
class BonReplicaServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        BonReplicaServiceKafkaResource kafkaResource = new BonReplicaServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bon-replica-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bon-replica-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
